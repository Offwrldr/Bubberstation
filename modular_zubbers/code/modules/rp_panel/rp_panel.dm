...
/datum/rp_panel
	...
	/// Current emote mode: "say", "emote", "subtle", "subtler", "subtler_antighost"
	var/emote_mode = "say"
	/// Chat history
	var/list/messages = list()
	/// Pending scene invites, keyed by ckey
	var/list/pending_invites = list()
	/// Shared scene location text
	var/location = ""
	/// Shared establishing details for the scene
	var/scene_details = ""
...
/datum/rp_panel/Destroy()
	if(holder)
		UnregisterSignal(holder, list(COMSIG_MOB_SAY, COMSIG_MOB_EMOTE, COMSIG_MOB_LOGIN, COMSIG_QDELETING))
	holder = null
	participants = null
	messages = null
...
/datum/rp_panel/ui_data(mob/user)
	...
	data["emote_modes"] = list(
		...
		"subtle" = "subtle",
		"subtler" = "subtler",
		"subtler_antighost" = "subtler anti-ghost"
	)
...
/datum/rp_panel/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("set_emote_mode")
			...
			if(new_mode in list("say", "emote", "subtle", "subtler", "subtler_antighost"))
				emote_mode = new_mode
				. = TRUE
		...
		if("set_location")
			...
			if(isnull(new_location))
				return
			location = copytext_char(trim("[new_location]"), 1, MAX_MESSAGE_LEN)
			sync_scene_context()
			. = TRUE

		if("set_scene_details")
			var/new_details = params["details"]
			if(isnull(new_details))
				return
			scene_details = copytext_char(trim("[new_details]"), 1, MAX_MESSAGE_LEN)
			sync_scene_context()
			. = TRUE
		...
		if("trigger_interaction")
			...
			if(interaction_component.body_relay && !can_see(holder, target))
				found_interaction.act(holder, target, interaction_component.body_relay, available_attitude, submissive_mode == "submissive")
			else
				found_interaction.act(holder, target, null, available_attitude, submissive_mode == "submissive")
			...
			if(interaction_message && length(interaction_message))
				...
				var/list/message_entry = list(
					...
					"mode" = found_interaction.lewd ? "subtle" : "emote",
		...
		if("invite_participant")
			...
			if(!target || QDELETED(target))
				return
			invite_living(target)
			. = TRUE

		if("add_participant")
			// Kept as the Join implementation; prefer invite_participant from the UI
			var/mob_ref = params["ref"]
			var/mob/living/target = locate(mob_ref)
			if(!target || QDELETED(target))
				return
			complete_add_participant(target)
			. = TRUE
		...
		if("send_message")
			...
			switch(emote_mode)
				if("say")
					holder.say(message)
					append_scene_message(message_entry)
...
/datum/rp_panel/proc/invite_living(mob/living/target)
	...
	if(isanimal(target) || istype(target, /mob/living/basic/pet))
		if(!iscyborg(target) && !istype(target, /mob/living/carbon/human/blowup_doll))
			to_chat(holder, span_warning("You cannot invite simplemobs or station pets to the scene."))
			return FALSE
	if(target == holder)
		return FALSE
	for(var/datum/weakref/participant_ref as anything in participants)
...
/datum/rp_panel/proc/complete_add_participant(mob/living/target)
	if(!target || QDELETED(target) || QDELETED(src) || QDELETED(holder))
		return FALSE
	if(isanimal(target) || istype(target, /mob/living/basic/pet))
		if(!iscyborg(target) && !istype(target, /mob/living/carbon/human/blowup_doll))
			return FALSE
	...
	target.rp_panel.location = location
...
/datum/rp_panel/proc/sync_scene_context()
	for(var/datum/weakref/participant_ref as anything in participants)
		var/mob/living/participant = participant_ref.resolve()
		if(participant && !QDELETED(participant) && participant.rp_panel)
			participant.rp_panel.location = location
			participant.rp_panel.scene_details = scene_details
			SStgui.update_uis(participant.rp_panel)
	SStgui.update_uis(src)
...
/datum/rp_panel/proc/send_subtler_message(message, range)
	...
	for(var/mob/viewer as anything in in_view)
		if(istype(viewer, /mob/eye/camera/ai) || (!viewer.client && viewer != holder))
			continue
		if(!isliving(viewer))
			continue
...
/datum/rp_panel/proc/on_emote(mob/living/source, datum/emote/emote, act, type_override, message, intentional)
	...
	if(emote_key == "subtle" || emote_key == "subtler")
		...
		// Determine mode
		var/mode = (emote_key == "subtle") ? "subtle" : "subtler"
...
/mob/living/proc/open_rp_panel_proc(mob/living/target_mob, mob/living/caller = null)
	...
	// If clicking on another player (not yourself), invite them if in range
	if(living_user != target_mob && isliving(target_mob))
		living_user.rp_panel.invite_living(target_mob)
...
/mob/living/verb/open_rp_panel()
	set name = "Scene Assistant"
	set category = "IC"
	set desc = "Open the Scene Assistant for collaborative roleplay"
	set hidden = FALSE // Ensure it's visible

	open_rp_panel_proc(src)

// Admin verb version - appears in admin context menu
ADMIN_VERB_ONLY_CONTEXT_MENU(open_rp_panel_admin, R_ADMIN, "Scene Assistant", mob/living/target in world)
