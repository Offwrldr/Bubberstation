...
export const RpPanel = (props) => {
  ...
  useEffect(() => {
    if (!showManageSelf) {
      ...
      const existingStyle = document.getElementById('rp-panel-manage-self-backdrop-fix');
      if (existingStyle) {
    ...
    const fixBackdrop = () => {
      ...
      allDivs.forEach((div) => {
        ...
        if (
          ...
          (style.zIndex === '1000' || style.zIndex === '1001' || parseInt(style.zIndex) >= 1000) &&
    ...
    return () => {
      const existingStyle = document.getElementById('rp-panel-manage-self-dropdown-fix');
      if (existingStyle) {
        existingStyle.remove();
      }
      const existingBackdropStyle = document.getElementById('rp-panel-manage-self-backdrop-fix');
  ...
  useEffect(() => {
    ...
    if (buttonBackground || buttonText) {
      css += `
        ...
        /* Exclude attitude buttons - they keep their original colors */
        .attitude-section .Button {
  ...
  useEffect(() => {
    ...
    const findScrollableContainer = () => {
      if (messagesEndRef.current) {
        ...
        while (parent) {
          if (parent.classList.contains('Section__content') || parent.scrollHeight > parent.clientHeight) {
  ...
  useEffect(() => {
    if (messages.length > prevMessagesLength.current && isAtBottom && messagesEndRef.current) {
      setTimeout(() => {
        if (messagesEndRef.current) {
            ...
            <Section fill scrollable>
              <Stack fill vertical>
                <>
                          ...
                          <Stack.Item>
                            <Stack>
                              ...
                              <Stack.Item grow ml={1}>
                                <Stack vertical>
                                  <Stack.Item>
                                    <Box bold fontSize="1.1em">{selected_participant.name}</Box>
                                  </Stack.Item>
                                  {selected_participant.details && selected_participant.details.length > 0 && (
                                    <Stack.Item>
                                      <Box
                                        style={{
                                      ...
                                      >
                                        {selected_participant.details.map((detail, index) => {
                                          // Bold keywords in details
                                          const keywords = ['hands', 'barefoot', 'mouth', 'uncovered', 'covered', 'neutral', 'gentle', 'hard', 'rough', 'naked', 'topless', 'bottomless', 'penis', 'vagina', 'anus', 'breasts', 'testicles', 'socks', 'feet'];
                                          let formattedDetail = detail;
                                          keywords.forEach((keyword) => {
                                            const regex = new RegExp(`\\b${keyword}\\b`, 'gi');
                                            formattedDetail = formattedDetail.replace(regex, (match) => `<b>${match}</b>`);
                                          });
                                          return (
                                            <Box
                                              key={index}
                            ...
                            <Stack.Item mt={1}>
                              <Stack fill>
                                <Stack.Item grow basis={0}>
                                  <Stack vertical>
                                    <Stack.Item>
                                      <Box fontSize="0.9em" style={{ textAlign: 'left' }}>
                                        ERP:
                                      </Box>
                                    </Stack.Item>
                                    <Stack.Item>
                                      <Box fontSize="0.9em" bold style={{ textAlign: 'left' }}>
                                        {selected_participant.erp_status_display || 'NO'}
                                      </Box>
                                    </Stack.Item>
                                  </Stack>
                                </Stack.Item>
                                <Stack.Item grow basis={0}>
                                  <Stack vertical>
                                    ...
                                    <Stack.Item>
                                      <Box fontSize="0.9em" bold style={{ textAlign: 'left' }}>
                                        {selected_participant.hypno_status_display || 'NO'}
                                      </Box>
                                    </Stack.Item>
                                  </Stack>
                                </Stack.Item>
                                <Stack.Item grow basis={0}>
                                  <Stack vertical>
                                    ...
                                    <Stack.Item>
                                      <Box fontSize="0.9em" bold style={{ textAlign: 'left' }}>
                                        {selected_participant.vore_status_display || 'NO'}
                                      </Box>
                                    </Stack.Item>
                                  </Stack>
                                </Stack.Item>
                                <Stack.Item grow basis={0}>
                                  <Stack vertical>
                                    ...
                                    <Stack.Item>
                                      <Box fontSize="0.9em" bold style={{ textAlign: 'left' }}>
                                        {selected_participant.noncon_status_display || 'NO'}
                                      </Box>
                                    </Stack.Item>
                                  </Stack>
                                </Stack.Item>
                                <Stack.Item grow basis={0}>
                                  <Stack vertical>
                                    ...
                                    <Stack.Item>
                                      <Box fontSize="0.9em" bold style={{ textAlign: 'left' }}>
                                        {selected_participant.erp_mechanics_display || 'NONE'}
                                      </Box>
                                    </Stack.Item>
                                  </Stack>
                                </Stack.Item>
                      ...
                      <Stack>
                        <Stack.Item grow>
                          <Button
                            ...
                            onClick={() => act('set_approach_mode', { approach: 'gentle' })}
                            style={approach_mode === 'gentle' ? {
                        ...
                        <Stack.Item grow>
                          <Button
                            ...
                            onClick={() => act('set_approach_mode', { approach: 'neutral' })}
                            style={approach_mode === 'neutral' ? {
                              'background-color': '#fff',
                              'color': '#000',
                            } : {}}
                          >
                        ...
                        <Stack.Item grow>
                          <Button
                            ...
                            onClick={() => act('set_approach_mode', { approach: 'hard' })}
                            style={approach_mode === 'hard' ? {
                              'background-color': '#fff',
                              'color': '#000',
                            } : {}}
                          >
                        ...
                        {show_erp_approaches !== false && (
                          <Stack.Item grow>
                            <Button
                              ...
                              onClick={() => act('set_approach_mode', { approach: 'rough' })}
                              style={approach_mode === 'rough' ? {
                                'background-color': '#fff',
                                'color': '#000',
                              } : {}}
                            >
                            ...
                            verb_mode_data.categories.map((category) => (
                              <Stack.Item key={category}>
                                <Collapsible title={category}>
                                  <Section fill>
                                    <Box mt={0.2}>
                                      {verb_mode_data.interactions[category]?.map((interaction) => (
                                        <Button
                                          key={interaction}
                                          width="150.5px"
                                          lineHeight={1.75}
                                          disabled={verb_mode_data.block_interact}
                                          color={
                                            verb_mode_data.block_interact
                                              ? 'grey'
                                              : verb_mode_data.colors[interaction] || 'blue'
                                          }
                                          content={interaction}
                                          tooltip={verb_mode_data.descriptions[interaction]}
                  ...
                  {verb_mode_data?.lewd_slots && verb_mode_data.lewd_slots.length > 0 && (
                    <Stack.Item>
                              ...
                              <Stack>
                                    ...
                                    {participant.is_typing && (
                                      <Stack.Item>
                                        <Box color="label" italic>
                                          <Box as="span" bold>{participant.name}</Box>
                                          {' is typing '}
                                          <TypingDots />
                                        </Box>
                                      </Stack.Item>
                                ...
                                <Stack.Item>
                                  <Button
                                    ...
                                    color="red"
                                    onClick={() =>
                                      act('remove_participant', { ref: participant.ref })
                                    }
                                  >
                                    Remove
                                  </Button>
                      ...
                      <Stack.Item>
                        <Box
                          style={{
                            ...
                            padding: '4px 8px',
                            'background-color': currentTheme.statusBarBackground || '#1e1e1e',
                            display: 'flex',
                            'align-items': 'center',
                        ...
                        >
                          ...
                          />
                          <Stack style={{ position: 'relative', 'z-index': 1, width: '100%' }}>
                            <Stack.Item grow />
                            <Stack.Item>
                              <Stack>
                                <Stack.Item>
                                  <Box
                                    style={{
                                      'font-size': '14px',
                                      color: currentTheme.statusBarText || '#fff',
                                      'margin-right': '8px',
                                    }}
                                  >
                                    <Icon name="heart" />
                        ...
                        >
                          <Box
                            style={{
                              ...
                              'background-color': 'rgba(209, 156, 217, 0.3)',
                              'z-index': 0,
                            }}
                          />
                          <Stack style={{ position: 'relative', 'z-index': 1, width: '100%' }}>
                      ...
                      >
                        <Stack>
                                ...
                                <Stack.Item>
                                  <Box
                                    ...
                                    style={{
                                      ...
                                      'border-radius': '4px',
                                      'margin-right': '8px',
                                      cursor: msg.ref ? 'pointer' : 'default',
                          ...
                          <Stack.Item grow>
                            {msg.mode === 'say' ? (
                              <Box>
                                <Box as="span" bold>{msg.name}</Box>
                                <Box as="span"> says, "{decodeHtmlEntities(msg.message)}"</Box>
                              </Box>
                            ) : msg.mode === 'whisper' ? (
                              <Box>
                                <Box as="span" bold>{msg.name}</Box>
                                <Box as="span"> whispers, "{decodeHtmlEntities(msg.message)}"</Box>
                              </Box>
                            ) : (
                              <Box>
                                <Box as="span" bold>{msg.name} </Box>
                                <Box as="span">{decodeHtmlEntities(msg.message)}</Box>
                              </Box>
                            )}
                            <Stack>
                              <Stack.Item grow>
                          ...
                          <Box
                            ...
                            onInput={(e) => {
                              ...
                              if (value.length <= MAX_MESSAGE_LENGTH) {
                                ...
                                setMessageInput(value.substring(0, MAX_MESSAGE_LENGTH));
                              }
                            }}
                            onKeyDown={(e) => {
                            ...
                            style={{
                              ...
                              'background-color': currentTheme.inputBackground || '#1e1e1e',
                    ...
                    <Stack.Item>
                      <Stack fill>
                        <Stack.Item>
                          ...
                          >
                            {messageInput ? messageInput.length : 0} / {MAX_MESSAGE_LENGTH} characters
                          </Box>
                        </Stack.Item>
                        <Stack.Item grow />
                        <Stack.Item>
                          <Dropdown
                            width="150px"
                            selected={emote_mode}
                            options={Object.entries(emote_modes).map(([key, label]) => ({
                              value: key,
                              displayText: label,
                            }))}
    ...
    {showParticipantManagement && (
      <Modal width="500px" style={{ zIndex: 1000 }}>
        <Section
          ...
          buttons={
            <Button icon="times" onClick={() => setShowParticipantManagement(false)}>
              Close
            </Button>
          }
        >
          <Section title="Available Players (5x5 range)" scrollable maxHeight="300px">
      ...
      <Modal width="600px" style={{ zIndex: 1001, position: 'relative' }}>
        <Section
          ...
          >
              ...
              <Section title="Character Prefs">
                <Stack fill vertical>
                  ...
                  <Stack.Item>
                    <Stack>
                      <Stack.Item>
                        <Box fontSize="0.9em" style={{ minWidth: '120px' }}>ERP:</Box>
                      </Stack.Item>
                      <Stack.Item grow>
                        <Stack wrap>
                          {[
                            ...
                            <Stack.Item key={option} mr={0.3} mb={0.3}>
                              <Button
                                selected={self_preferences?.erp_status === option}
                                onClick={() => {
                  ...
                  <Stack.Item>
                    <Stack>
                      <Stack.Item>
                        <Box fontSize="0.9em" style={{ minWidth: '120px' }}>Noncon:</Box>
                      </Stack.Item>
                      <Stack.Item grow>
                        <Stack wrap>
                          {[
                            ...
                            <Stack.Item key={option} mr={0.3} mb={0.3}>
                              <Button
                                selected={self_preferences?.erp_status_nc === option}
                                onClick={() => {
                                  act('set_self_preference', {
                  ...
                  <Stack.Item>
                    <Stack>
                      <Stack.Item>
                        <Box fontSize="0.9em" style={{ minWidth: '120px' }}>Vore:</Box>
                      </Stack.Item>
                      <Stack.Item grow>
                        <Stack wrap>
                          {[
                            ...
                            <Stack.Item key={option} mr={0.3} mb={0.3}>
                              <Button
                                selected={self_preferences?.erp_status_v === option}
                                onClick={() => {
                                  act('set_self_preference', {
                  ...
                  <Stack.Item>
                    <Stack>
                      <Stack.Item>
                        <Box fontSize="0.9em" style={{ minWidth: '120px' }}>Hypnosis:</Box>
                      </Stack.Item>
                      <Stack.Item grow>
                        <Stack wrap>
                          {[
                            ...
                            <Stack.Item key={option} mr={0.3} mb={0.3}>
                              <Button
                                selected={self_preferences?.erp_status_hypno === option}
                                onClick={() => {
                                  act('set_self_preference', {
                  ...
                  <Stack.Item>
                    <Stack>
                      <Stack.Item>
                        <Box fontSize="0.9em" style={{ minWidth: '120px' }}>Mechanics:</Box>
                      </Stack.Item>
                      <Stack.Item grow>
                        <Stack wrap>
                          {[
                            ...
                            <Stack.Item key={option} mr={0.3} mb={0.3}>
                              <Button
                                selected={self_preferences?.erp_status_mechanics === option}
                                onClick={() => {
                                  act('set_self_preference', {
            ...
            {/* Genital Visibility */}
            {self_preferences?.genitals && self_preferences.genitals.length > 0 && (
              <Stack.Item mt={1}>
                        ...
                        <Stack>
                          ...
                          <Stack.Item>
                            <Button
                              ...
                              onClick={() => act('set_genital_visibility', {
